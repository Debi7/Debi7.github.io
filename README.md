...
### Math is loaded but never typeset

Found while checking the change above, and not caused by it. `Head.astro` loads KaTeX from a CDN and `site.ts` calls `renderMathInElement` with the theme's delimiters, but `remark-math` has already consumed the dollars at build time: `$E = mc^2$` reaches the browser as `<code class="language-math math-inline">E = mc^2</code>`, and auto-render scans text for dollars. Measured on a temporary post with an inline and a display formula: zero `.katex` nodes, while `typeof renderMathInElement` was `function`. So the CDN and the call are healthy and the Markdown pipeline is what breaks it.

No page on the site uses math today, so nothing is visibly broken. The fix is a decision the owner has not made yet - `rehype-katex` (typeset at build, no CDN) or dropping `remark-math` (dollars reach the browser, as they do in Hugo); `CLAUDE.md` carries it under "Open questions for the owner" with the recommendation.

While looking at that, the `onload="renderMathInElement(document.body);"` attribute on the auto-render tag was removed: it typeset the whole document with KaTeX's default delimiters and `site.ts` then typeset it again with the theme's, and an inline handler is the one thing a Content-Security-Policy cannot allow without `unsafe-inline`. The remaining call is guarded, so an unreachable CDN can no longer take the theme switch and the menu down with it.

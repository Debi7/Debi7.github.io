...
- The tag cloud under the home carousel, and the round menu links hidden below 640px (2026-09-22). Hugo has neither: its home page has no cloud at all, and it prints the row of round links at every width. The owner asked for the cloud after a reference site he gave, and in the same conversation for both blocks to disappear where the header's menu becomes a hamburger, so `/` departs from the Hugo screenshot in a second way on top of the carousel offset above. README, "The tag cloud under the carousel", carries the reasoning; `src/components/TagCloud.astro` carries the rest, including why it is HTML and not the canvas the reference uses.

...
- Anything that reproduces a Hugo quirk gets a comment saying so, otherwise the next session "fixes" it. Examples already in the tree: the duplicated `class` attribute behind `Menu.astro`, the never-matching `$isCurrentTag` in `tags/[slug]/[...page].astro` (it was `tags/[slug].astro` until the lists got pagination on 2026-09-19), and, until that evening, the tag and category hrefs without a trailing slash - they have one now, because the slash-less address is a 404 on Astro's own dev and preview servers under `trailingSlash: "always"`, so the quirk was visible after all (REVIEW-FIXES.md, section 5).

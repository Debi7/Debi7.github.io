...
- The post lists are paginated (2026-09-19, owner: 5 a page, `pagination.pageSize` in `src/config.ts`). The Posts list is paginated by year, as the owner asked: `/posts/<year>/` and `/posts/<year>/page/<n>/` hold only that year, Previous and Next never leave it, and `/posts/` shows the newest year; a year switcher sits under the title. Tag and category pages are paginated across all years; splitting them by year was judged not worth it (small tags, unlinked category pages). Every list has numbered page links between Previous and Next, two pages either side of the current one from 640px up and one below. The buttons are `list.html`'s on every list, the tag pages included (owner, 2026-09-19, after the reviewer noticed that `tag.html`'s were round and white in dark mode); the theme's originals are in a comment in the tag template. A post dated in the future is left out of the published site until its date (owner, 2026-09-19, as Hugo's `buildFuture = false`); `npm run dev` shows it. The order of every list, the future-date filter and the page cutting live in `src/lib/posts.ts` (`getPosts`, `groupByYear`, `paginateList`, `pageNumbers`, `yearLinks`); the list routes are rest-parameter routes, `posts/[...page].astro`, `tags/[slug]/[...page].astro` and `categories/[slug]/[...page].astro`. A post must never be named after a year (`2025.md`). The theme's list templates loop over `.Pages`, not `.Paginator.Pages`; that bug is deliberately not ported. A stress test with up to 100 pages a year passed; see `PAGINATION.md`.

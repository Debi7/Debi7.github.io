...
- Anything that reproduces a Hugo quirk gets a comment saying so, otherwise the next session "fixes" it. Examples already in the tree: the duplicated `class` attribute behind `Menu.astro`, the never-matching `$isCurrentTag` in `tags/[slug]/[...page].astro` (it was `tags/[slug].astro` until the lists got pagination on 2026-09-19), the tag hrefs without a trailing slash.
